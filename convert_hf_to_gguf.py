...
@ModelBase.register("LlamaForCausalLM_EAGLE")
class LlamaModel(TextModel):
    # model_arch = gguf.MODEL_ARCH.LLAMA
    model_arch = gguf.MODEL_ARCH.EAGLE
    undo_permute = True
    ...
    def generate_extra_tensors(self) -> Iterable[tuple[str, Tensor]]:
        ...
        # Load lm_head.weight and model.norm.weight from base model for LM HEAD SHARING
        if self.base_model_dir is not None:
            logger.info(f"EAGLE: Loading lm_head from base model: {self.base_model_dir}")
            base_safetensors = sorted(self.base_model_dir.glob("model*.safetensors"))
            base_pytorch = sorted(self.base_model_dir.glob("pytorch_model*.bin"))

            tensors_to_find = {"lm_head.weight"}
            found: dict[str, Tensor] = {}

            if len(base_safetensors) > 0:
                from safetensors import safe_open
                for sf_path in base_safetensors:
                    with safe_open(sf_path, framework="pt", device="cpu") as f:
                        for key in f.keys():
                            if key in tensors_to_find:
                                found[key] = f.get_tensor(key)
                                logger.info(f"EAGLE: Loaded {key} from {sf_path.name}, shape={found[key].shape}")
                    if len(found) == len(tensors_to_find):
                        break
            elif len(base_pytorch) > 0:
                for pt_path in base_pytorch:
                    pt_data = torch.load(str(pt_path), map_location="cpu", weights_only=True)
                    for key in list(pt_data.keys()):
                        if key in tensors_to_find:
                            found[key] = pt_data[key]
                            logger.info(f"EAGLE: Loaded {key} from {pt_path.name}, shape={found[key].shape}")
                    if len(found) == len(tensors_to_find):
                        break
            ...
            # output_norm skipped - not needed for EAGLE draft model
        else:
            if self.tensor_names is not None:
                has_lm_head = any("lm_head" in n or "output.weight" in n for n in self.tensor_names)
                if not has_lm_head:
                    logger.warning("EAGLE: No lm_head found. Use --base-model to specify the base model for LM HEAD SHARING.")
