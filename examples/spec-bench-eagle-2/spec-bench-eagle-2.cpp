// Spec-Bench runner for EAGLE-2 speculative decoding.
// Based on speculative-eagle-2.cpp — loads GPU target/draft models once,
// then runs multiple prompts sequentially.
//
// Usage:
//   ./llama-spec-bench-eagle-2 --bench-file prompts.jsonl [all normal speculative-eagle flags]
//   ./llama-spec-bench-eagle-2 --bench-file prompts.jsonl -n 512 -m model.gguf -md draft.gguf ...
//
...
#define SPEC_BENCH_BACKEND_LABEL "Spec-Bench EAGLE-2"
...
static std::vector<size_t> TopK(const std::vector<float> & data, size_t k) {
    if (k > data.size()) {
        k = data.size();
    }
...
int main(int argc, char ** argv) {
    ...
    for (int i = 0; i < argc; ++i) {
        ...
        if (arg == "--bench-file" && i + 1 < argc) {
            ...
            n_depth = std::atoi(argv[++i]);
        } else if (arg == "--top-k" && i + 1 < argc) {
            draft_top_k = std::atoi(argv[++i]);
            expand_k = draft_top_k;
        } else if (arg == "--expand-k" && i + 1 < argc) {
            expand_k = std::atoi(argv[++i]);
        } else if (arg == "--rerank-k" && i + 1 < argc) {
            rerank_k = std::atoi(argv[++i]);
    ...
    if (bench_file.empty()) {
        ...
        fprintf(stderr, "\nEAGLE-2 draft tree options:\n");
        fprintf(stderr, "  --n-depth N            Draft tree depth (default: 5)\n");
        fprintf(stderr, "  --top-k N              Draft tree Top-K and default Expand-K (default: 10)\n");
        fprintf(stderr, "  --expand-k N           Branch expansion K (default: 10)\n");
        fprintf(stderr, "  --rerank-k N           Token-level reranking K (default: 59)\n");
        fprintf(stderr, "  --rerank               Enable token-level reranking (default)\n");
        fprintf(stderr, "  --no-rerank            Disable token-level reranking\n");
        return 1;
    }

    if (n_depth <= 0 || draft_top_k <= 0 || expand_k <= 0 || rerank_k <= 0) {
        fprintf(stderr,
                "Error: --n-depth, --top-k, --expand-k, and --rerank-k must all be positive\n");
        return 1;
    }
    ...
    const auto & tgt_vocab_map = model_tgt->vocab_map;
    const auto & dft_vocab_map = model_dft->vocab_map;
    const bool target_has_vocab_trim = !tgt_vocab_map.empty();
    const bool has_vocab_trim = !dft_vocab_map.empty();

    if (has_vocab_trim) {
        fprintf(stderr,
                "[Spec-Bench EAGLE-2] Draft vocab trimming active: %zu entries in vocab_map (output_vocab_size=%u)\n",
                dft_vocab_map.size(), model_dft->hparams.n_vocab_output);
        fprintf(stderr, "[Spec-Bench EAGLE-2] LM head sharing disabled because draft vocab is trimmed\n");
    } else if (target_has_vocab_trim) {
        fprintf(stderr,
                "[Spec-Bench EAGLE-2] Target vocab trimming active: %zu entries in vocab_map (output_vocab_size=%u)\n",
                tgt_vocab_map.size(), model_tgt->hparams.n_vocab_output);
        fprintf(stderr, "[Spec-Bench EAGLE-2] LM head sharing disabled because target output vocab is trimmed but draft output vocab is full\n");
    } else {
        ...
        if (!dft_output) {
            fprintf(stderr, "Error: draft model output tensor is NULL\n");
            return 1;
        }
        if (tgt_output->ne[0] != dft_output->ne[0] || tgt_output->ne[1] != dft_output->ne[1]) {
            fprintf(stderr,
                    "[Spec-Bench EAGLE-2] LM head sharing disabled because output tensor shapes differ: target=[%ld,%ld], draft=[%ld,%ld]\n",
                    tgt_output->ne[0], tgt_output->ne[1], dft_output->ne[0], dft_output->ne[1]);
        } else {
            ...
            fprintf(stderr, "[Spec-Bench EAGLE-2] LM head sharing: OK\n");
        }
    }
    ...
    fprintf(stderr,
            "[Spec-Bench EAGLE-2] Draft tree: n_depth=%d, top_k=%d, expand_k=%d, rerank=%s, rerank_k=%d\n",
            n_depth, draft_top_k, expand_k, rerank ? "on" : "off", rerank_k);
    fprintf(stderr, "[Spec-Bench EAGLE-2] Models loaded. Starting benchmark...\n\n");
    ...
    for (size_t prompt_idx = idx_start; prompt_idx < idx_end; ++prompt_idx) {
        ...
        if (tgt_prefill_ok && (sliced_data.size() < expected_sliced || backup_data.size() < expected_backup)) {
            fprintf(stderr,
                    "  SKIP: hidden state size mismatch (hidden_dim=%d, n_prefill=%d, sliced=%zu, backup=%zu)\n",
                    hidden_dim, n_prefill, sliced_data.size(), backup_data.size());
            tgt_prefill_ok = false;
        }
        ...
        for (int chunk_start = 0; chunk_start < n_eagle_tokens; chunk_start += n_prefill_chunk_dft) {
            const int chunk_size = std::min(n_prefill_chunk_dft, n_eagle_tokens - chunk_start);
        ...
        // ====================================================================
        // Speculative decode loop using the EAGLE-2 dynamic draft tree.
        // ====================================================================
        while (true) {
            ...
            for (int i = 0; i < n_draft_max; ++i) {
                ...
                std::fill(column_scores.begin(), column_scores.end(), 0.0f);

                if (batch_tgt.n_tokens >= n_draft_max) {
                    break;
                }

                for (int s = 0; s < n_seq_dft; ++s) {
                    drafts[s].skip =
                        std::find(topk_indices.begin(), topk_indices.end(), (size_t) s) == topk_indices.end();
                }

                std::vector<float> temp;
                std::vector<llama_token> ids;
                std::vector<int> seqs;
                std::vector<std::vector<llama_token_data>> token_dists;
                ...
                for (int s = 0; s < n_seq_dft; ++s) {
                    ...
                    const int split_limit = std::min(expand_k, (int) cur_p->size);
                    ...
                    for (int ii = 0; ii < n_seq_dft; ++ii) {
                        temp_i_batch_dft[ii] = drafts[ii].i_batch_dft;
                    }

                    for (int is = 0; is < (int)sa.size(); ++is) {
                        const llama_token id = cur_p->data[is].id;
                        ids.push_back(id);
                        seqs.push_back(sa[is]);
                        token_dists.emplace_back(cur_p->data, cur_p->data + cur_p->size);
                    }
                }
                ...
                for (int is = 0; is < (int) ids.size(); ++is) {
                    ...
                    const int s = seqs[is];
                    const int proposed_position = (int) drafts[s].tokens.size();
                    ...
                    drafts[s].dists.push_back(std::move(token_dists[is]));
                    ...
                    const bool selected_for_next_step =
                        topk_indices.size() == 1 ||
                        std::find(topk_indices.begin(), topk_indices.end(), (size_t) s) != topk_indices.end();
                    if (selected_for_next_step) {
                ...
                T_d.push_back((dft_t1 - dft_t0) / 1000.0f);
                ++n_past_cur;
                ++n_drafted;
            }

            if (rerank) {
                const int total_drafted_tokens = batch_tgt.n_tokens - 1;
                if (total_drafted_tokens > rerank_k) {
                    ...
                    std::vector<token_score> token_scores;
                    token_scores.reserve(total_drafted_tokens);
                    for (int t = 1; t < batch_tgt.n_tokens; ++t) {
                        const int depth = batch_tgt.pos[t] - n_past_tgt - 1;
                        const int s = batch_tgt.seq_id[t][0];
                        const float score =
                            depth >= 0 && depth < n_depth && s >= 0 && s < n_seq_dft ?
                            scores[s][depth] : 0.0f;
                        token_scores.push_back({t, score, depth});
                    }

                    std::sort(token_scores.begin(), token_scores.end(), [](const token_score & a, const token_score & b) {
                        if (a.score != b.score) {
                    ...
                    surviving_tokens.insert(0);
                    const int keep_count = std::min(rerank_k, (int) token_scores.size());
                    for (int ti = 0; ti < keep_count; ++ti) {
                        surviving_tokens.insert(token_scores[ti].t_idx);
                    }
                    ...
                    for (int s = 0; s < n_seq_dft; ++s) {
                        ...
                        for (size_t it = 0; it < drafts[s].i_batch_tgt.size(); ++it) {
                            const int old_idx = drafts[s].i_batch_tgt[it];
                            if (old_idx >= 0 &&
                                old_idx < (int) old_to_new_idx.size() &&
                                old_to_new_idx[old_idx] != -1) {
                                new_i_batch_tgt.push_back(old_to_new_idx[old_idx]);
                                if (it < drafts[s].tokens.size()) {
                                    new_tokens.push_back(drafts[s].tokens[it]);
                                }
                                if (it < drafts[s].dists.size()) {
                                    new_dists.push_back(std::move(drafts[s].dists[it]));
                                }
                            }
                        }

                        if (new_i_batch_tgt.size() <= 1) {
                            drafts[s].i_batch_tgt = std::move(new_i_batch_tgt);
                            drafts[s].tokens = std::move(new_tokens);
                            drafts[s].dists = std::move(new_dists);
                            drafts[s].active = false;
                            if (s > 0 && drafts[s].smpl) {
                    ...
                    for (int t = 0; t < batch_tgt.n_tokens; ++t) {
                        int valid_seqs = 0;
                        for (int p = 0; p < batch_tgt.n_seq_id[t]; ++p) {
                            const int s = batch_tgt.seq_id[t][p];
                            if (s == 0 || (s >= 0 && s < n_seq_dft && drafts[s].active)) {
