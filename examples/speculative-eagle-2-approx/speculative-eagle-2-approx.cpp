...
int main(int argc, char ** argv) {
    ...
    if (has_vocab_trim) {
        ...
        LOG_INF("[EAGLE] Target vocab trimming active: %zu entries in vocab_map (output_vocab_size=%u)\n",
                tgt_vocab_map.size(), model_tgt->hparams.n_vocab_output);
        LOG_INF("[EAGLE] LM head sharing disabled because target output vocab is trimmed but draft output vocab is full\n");
    } else {
        ...
        if (!dft_output) {
            LOG_ERR("Draft model output tensor is NULL - cannot perform LM Head Sharing\n");
            return 1;
        }
        if (tgt_output->ne[0] != dft_output->ne[0] || tgt_output->ne[1] != dft_output->ne[1]) {
            LOG_INF("[EAGLE] LM head sharing disabled because output tensor shapes differ: target=[%ld,%ld], draft=[%ld,%ld]\n",
    ...
    while (true) {
        ...
        step_start_us = step_verify_logic_start;
        ...
        // Per-step TBT 기록 (verification 시작 ~ 다음 target decode 직후)
        {
            ...
            cum_tbt_us     += step_total_us;
            cum_tbt_tokens += step_n_tokens;
            step_log.push_back({ step_total_us, step_n_tokens, cum_tbt_us, cum_tbt_tokens });
        }
    ...
    if (!step_log.empty()) {
        ...
        double max_tbt = 0.0;
        size_t cnt = 0;
        for (size_t i = start_idx; i < step_log.size(); ++i) {
            if (step_log[i].n_tokens <= 0) continue;
            const double tbt_ms = (step_log[i].step_total_us / 1000.0) / step_log[i].n_tokens;
            sum_tbt += tbt_ms;
            sum_sq  += tbt_ms * tbt_ms;
            if (tbt_ms < min_tbt) min_tbt = tbt_ms;
            if (tbt_ms > max_tbt) max_tbt = tbt_ms;
            cnt++;
        }
        if (cnt > 0) {
            const double mean_tbt = sum_tbt / cnt;
            const double var_tbt  = sum_sq / cnt - mean_tbt * mean_tbt;
            const double std_tbt  = var_tbt > 0 ? std::sqrt(var_tbt) : 0.0;

            LOG_INF("\n");
            LOG_INF("Per-step TBT (excluding step 0):\n");
            LOG_INF("  steps measured: %zu\n", cnt);
            LOG_INF("  mean TBT     : %.3f ms/token\n", mean_tbt);
            LOG_INF("  stddev TBT   : %.3f ms/token\n", std_tbt);
            LOG_INF("  min  TBT     : %.3f ms/token\n", min_tbt);
            LOG_INF("  max  TBT     : %.3f ms/token\n", max_tbt);
        }

        if (!tbt_csv_path.empty()) {
            std::ofstream csv(tbt_csv_path);
            if (csv.is_open()) {
                csv << "step_idx,n_tokens,step_total_us,tbt_per_token_us,cum_tokens,cum_us\n";
                for (size_t i = 0; i < step_log.size(); ++i) {
                    const auto & e = step_log[i];
                    const double tbt_us = e.n_tokens > 0 ? (double) e.step_total_us / e.n_tokens : 0.0;
                    csv << i << "," << e.n_tokens << "," << e.step_total_us << ","
                        << (int64_t) tbt_us << "," << e.cum_tokens << "," << e.cum_us << "\n";
                }
                csv.close();
                LOG_INF("TBT CSV written: %s (%zu steps)\n", tbt_csv_path.c_str(), step_log.size());
            } else {
                LOG_ERR("failed to open TBT CSV path: %s\n", tbt_csv_path.c_str());
            }
        }
    }
