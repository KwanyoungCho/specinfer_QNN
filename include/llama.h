...
#define LLAMA_STATE_SEQ_FLAGS_PARTIAL_ONLY 1
    ...
    //
    // External logits injection (for custom backends such as QNN)
    //
    // Writes n_outputs rows of logits (each of size n_vocab) into the context's
    // internal output buffer so that llama_get_logits[_ith]() and
    // llama_sampler_sample() can consume them as usual.
    //
    // The logits pointer must contain contiguous data in row-major layout:
    //   [n_outputs][n_vocab]
    //
    LLAMA_API void llama_set_logits_external(
            struct llama_context * ctx,
            const float          * logits,
                     int32_t       n_outputs);
