...
bool LLMDecodeRunner::generate(const std::vector<int32_t>& prompt_tokens,
                               std::vector<int32_t>& generated_tokens) {
  // Start inference timing
  stats_.inference_start_ms = time_in_ms();

  if (prompt_tokens.empty()) {
    error_msg_ = "Empty prompt_tokens";
    return false;
  }

  // Copy prompt tokens so we can append generated ids as we go
  std::vector<int32_t> tokens = prompt_tokens;

  stats_.num_prompt_tokens = tokens.size();

  if (config_.log_level >= 1) {
    std::cout << "\n[Generate] Prompt tokens: " << tokens.size() << "\n";
  }
  ...
  std::cout << "[TTFT] Prefill latency: " << stats_.first_token_ms - stats_.inference_start_ms << " ms\n";
  ...
  if (config_.log_level >= 1) {
    std::cout << "[Prefill] First token id: " << next_token << "\n";
  }

  // Rearrange cache for decode (single-context only, multi-context does it internally)
  if (!config_.use_multi_context) {
    if (config_.log_level >= 1) {
  ...
  for (int gen_idx = 0; gen_idx < config_.max_gen_tokens - 1; ++gen_idx) {
    ...
    // Check EOS (hard-coded Llama special token ids)
    ...
    generated_tokens.push_back(token_out);
