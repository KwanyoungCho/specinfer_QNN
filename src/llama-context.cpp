...
void llama_context::synchronize() {
    // In vocab_only mode, no backend scheduler is created. In that case there is
    // nothing to synchronize and we can safely treat this as a no-op.
    // It's because QNN doesn't use gguf weights!
    if (!sched) {
        GGML_ASSERT(model.hparams.vocab_only);
...
void llama_set_logits_external(
           llama_context * ctx,
           const float   * logits,
                    int32_t n_outputs) {
    if (ctx == nullptr || logits == nullptr || n_outputs <= 0) {
        return;
    }

    const uint32_t n_outputs_max = ctx->output_reserve(n_outputs);
    if ((uint32_t) n_outputs > n_outputs_max) {
        return;
    }

    float * dst = ctx->get_logits();
    if (dst == nullptr) {
        return;
    }
    ...
    // set simple 0..n_outputs-1 mapping for output ids
    for (int32_t i = 0; i < n_outputs; ++i) {
        ctx->output_ids[i] = i;
    }

    ctx->n_outputs = n_outputs;
    ctx->output_swaps.clear();
}
...
int llama_context::decode_eagle(llama_batch & batch_inp, void * data) {
    GGML_ASSERT((!batch_inp.token && batch_inp.embd) || (batch_inp.token && !batch_inp.embd)); // NOLINT
