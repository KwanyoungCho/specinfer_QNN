...
LLMKVCacheManager::LLMKVCacheManager(const Metadata & metadata) :
    metadata_(metadata),
    total_cache_size_(0),
    cur_ar_len_(metadata.max_ar_len) {
    // Resize storage
...
LLMKVCacheManager::~LLMKVCacheManager() {
    ...
    for (auto & layer_k : k_cache_) {
        for (auto & head_k : layer_k) {
            if (head_k.input_buffer) {
                free(head_k.input_buffer);
            }
            if (head_k.output_buffer) {
                free(head_k.output_buffer);
            }
        }
    }
    for (auto & layer_v : v_cache_) {
        for (auto & head_v : layer_v) {
            if (head_v.input_buffer) {
                free(head_v.input_buffer);
            }
            if (head_v.output_buffer) {
                free(head_v.output_buffer);
            }
        }
    }
}
...
bool LLMKVCacheManager::allocate() {
    ...
    for (int32_t layer = 0; layer < metadata_.num_layers; ++layer) {
        for (int32_t head = 0; head < metadata_.num_heads; ++head) {
            ...
            if (!k_cache_[layer][head].input_buffer || !k_cache_[layer][head].output_buffer) {
                std::cerr << "[LLMKVCacheManager] Failed to allocate K cache for layer " << layer << ", head " << head
                          << "\n";
                return false;
            }
            ...
            if (!v_cache_[layer][head].input_buffer || !v_cache_[layer][head].output_buffer) {
                std::cerr << "[LLMKVCacheManager] Failed to allocate V cache for layer " << layer << ", head " << head
                          << "\n";
                return false;
            }
    ...
    // if (config_.log_level >= 2) {
    //     std::cout << "[LLMKVCacheManager] Allocation complete: " << (total_cache_size_ / 1024.0 / 1024.0) << " MiB\n";
    // }
    return true;
}
...
void LLMKVCacheManager::rearrange_value(KVCacheBuffer & cache, int32_t src_cache_len, int32_t dst_cache_len) {
...
void LLMKVCacheManager::rearrange_cache(int32_t src_ar_len, int32_t dst_ar_len) {
    ...
    if (src_ar_len == dst_ar_len) {
        std::cout << "[LLMKVCacheManager] Rearrange skipped (same AR len: " << src_ar_len << ")\n";
        return;
    }
    ...
    // if (config_.log_level >= 2) {
    //     std::cout << "[LLMKVCacheManager] Rearranging cache: AR " << src_ar_len << " → " << dst_ar_len << " (cache "
    //               << src_cache_len << " → " << dst_cache_len << ")\n";
    // }
